...
5개 핵심 컴포넌트의 유닛 테스트가 작성되었습니다:

- **StatCard.test.tsx** - 통계 카드 (props, 접근성, 그라디언트)
- **MemberCard.test.tsx** - 멤버 카드 (활동 상태, 시간 포맷, 권한)
- **Spinner.test.tsx** - 로딩 스피너 (크기, 접근성)
- **EmptyState.test.tsx** - 빈 상태 (아이콘, 설명, 접근성)
- **ErrorBoundary.test.tsx** - 에러 경계 (에러 캐치, 폴백 UI)
- **utils.test.ts** - API 클라이언트 유틸리티
...
- `jest.config.js` - Jest 설정 (경로 매핑, 커버리지)
- `jest.setup.js` - 테스트 환경 (모킹, 전역 설정)

### 2. E2E 테스트 (Playwright - 별도 문서)

통합 테스트는 루트의 `e2e/` 디렉터리 참조.
